/**
 * Upload-form validity for the notary/delivery/contact-phone fields on the
 * order form. Extracted from src/app/[locale]/dashboard/page.tsx so this
 * logic is independently testable — see __tests__/notary-delivery-validation.test.ts.
 *
 ...
 *
 * 2026-08-08 business change: a contact phone (deliveryPhone — field kept
 * under its original name to avoid renaming the DB column / Jira custom
 * field) is now required for ALL notarized orders regardless of pickup vs
 * delivery (WPO may need to reach the client about the document itself or
 * about pickup, not just courier logistics), and also for the Official
 * (certified) service level, which has no fulfillment/pickup concept at all.
 * deliveryAddress remains delivery-only.
 */
 ...
 * no-op for them). A contact phone is required for every notarized order,
 * pickup or delivery alike. Delivery additionally requires city + fulfillment
 * method + a non-whitespace address.
 *
...
export function isNotaryDeliveryValid(state: NotaryDeliveryFormState): boolean {
  ...
  if (!isFilledIn(state.deliveryPhone)) return false;
  if (!isDeliverySelected(state)) return true;
  return isFilledIn(state.deliveryAddress);
}

/**
 * Official (certified-translator) service level has no fulfillment/pickup
 * concept — it is a purely electronic handoff of the final PDF — but a
 * contact phone is still required so WPO can reach the client about the
 * document if a question comes up during processing.
 */
export function isOfficialContactPhoneValid(deliveryPhone: string): boolean {
  return isFilledIn(deliveryPhone);
}
