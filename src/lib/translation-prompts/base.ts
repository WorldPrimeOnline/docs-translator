...
Visual element markers must be emitted in the TARGET language of the translation.
For Russian target: use Russian bracket markers as defined in the reference set below.
...
function outputContractClause(outputMode: OutputMode): string {
  const base = `OUTPUT CONTRACT:
Return only clean Markdown suitable for official translation rendering.

Mandatory structure:
1. Official translated content.
2. Translator notes only if needed for unclear/illegible fields.
3. Mandatory section at the end:
   ## Описание нетекстовых элементов оригинала
   (or ## Description of non-text elements in the original for English target)

Never include implementation notes, model names, service names, or internal technical terms.
Never mention AI, OCR, Claude, Mistral, model, parser, renderer, fallback, JSON, Markdown rules, prompt rules, or internal service levels.

For every visual element in the source document, insert an inline bracket marker near its original position AND list it in the visual elements section.

Verification codes, QR contents, URLs, MRZ lines, barcodes and electronic approval strings must never appear as naked text. Place them inside a labeled bracket marker or table row.

Do not invent missing content. If OCR is uncertain, preserve the original visible value.

FORBIDDEN OUTPUT WORDING:
Do not write: "generated by AI", "machine translation", "OCR result", "source text", "target language", "translation prepared by service".`;

  if (outputMode === 'notarization_package') {
    return `${base}

TRANSLATOR REVIEW MODE:
This draft is for human translator review. You may add [ПРОВЕРИТЬ] / [CHECK] markers on fields that are ambiguous or OCR-uncertain. Do not add these in other modes.`;
  }
...
${outputContractClause(outputMode)}
