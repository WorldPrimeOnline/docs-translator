...
### 2026-07-13 — Locale-aware SEO metadata + sitewide removal of AI-translator brand positioning

**Decision:**  
Two related changes shipped together: (1) `src/lib/seo/site-metadata.ts` now generates locale-aware `<title>`/description/Open Graph/Twitter/canonical/hreflang for the homepage (`src/app/[locale]/page.tsx`) and a locale-aware fallback for routes without their own `generateMetadata` (`src/app/layout.tsx`), replacing the old static English "AI Document Translation" metadata; canonical/OG URLs use `https://www.wpotranslations.org` (verified live — the apex domain 307s to `www` at the Vercel domain level, so `NEXT_PUBLIC_SITE_URL`, which points at the apex, is stale for this purpose). (2) All public-facing AI/ИИ-translator brand positioning was removed from marketing copy across all 14 locales (footer tagline, homepage hero badge, trust pillar, security section, pricing feature bullets, order service-level description, order status label, hero mockup step label) — WPO must read as "online document translation," not an AI translator/GPT-wrapper. `scripts/i18n/find-forbidden-phrases.ts` gained an `AI_MARKETING_FORBIDDEN` rule set (AI-powered branding, AI translator/translation branding, neural network/machine translation branding, bare Mistral/Claude/GPT vendor names) that runs everywhere except `AI_MARKETING_ALLOWLIST` (`src/lib/legal/**`, each locale's `legal.json`, `src/app/api/**`) — legal disclosures and backend/OCR-provider code are exempt from this rule set only, not from the pre-existing false-promise `FORBIDDEN` set.

**Rationale:**  
Sharing `https://www.wpotranslations.org/` in Telegram/social previews showed English "AI Document Translation" copy regardless of locale — wrong positioning (WPO is not an AI translator) and not locale-aware. The same AI-translator framing had spread into homepage marketing copy (hero badge "AI-powered", trust pillar "AI-powered accuracy" naming Mistral/Claude by name, a security claim "Translation is performed by AI only"). Per product direction, AI/OCR may only be described as internal processing infrastructure (already correctly done in several FAQ answers — e.g. "processed automatically using OCR and AI; official translation is reviewed by a human translator" — which were deliberately left untouched), never as the brand's core description. Legal/consent/privacy content is required by law to name subprocessors (Mistral AI, Anthropic) and disclose AI/OCR processing in full, so it is explicitly exempted rather than caught by the new marketing-only rule set.

**Impacted files/docs:**  
`src/lib/seo/site-metadata.ts` (new), `src/app/layout.tsx`, `src/app/[locale]/page.tsx`, `src/lib/seo/__tests__/site-metadata.test.ts` (new), `src/i18n/__tests__/locales.test.ts` (new), `scripts/i18n/find-forbidden-phrases.ts`, `messages/{ru,en,kk,zh,ko,tj,uz,tk,mn,ky,es,th,de,tr}/{footer,home,pricing,common,checkout,order}.json`.

**Risks / caveats:**  
`messages/_legacy/*.json` (archived, unused — not in the `NAMESPACES` load list in `src/i18n/request.ts`, not scanned by `i18n:forbidden`) still contains old AI-branded copy; left untouched since it is never served. Several locale files (`es`, `ko`, `mn`, `tj`, `tk`) still carry pre-existing `TODO_I18N` raw-Russian placeholder text for unrelated FAQ entries (a pre-existing localization-completeness gap, not part of this cleanup's scope — those entries' AI-positioning content was still corrected, just not translated out of Russian). `AI_MARKETING_ALLOWLIST` exempts whole files/dirs (not line-level) — acceptable because the exemption list is small, explicit, and documented, and those files/dirs are still scanned by the base `FORBIDDEN` (false-promise) rule set.

---

### 2026-07-14 — Direct-to-R2 upload flow for order drafts (init/complete)

**Decision:**  
Replaced the single-request multipart upload for the anonymous pre-checkout draft wizard with a two-step direct-to-R2 flow. POST /api/order-drafts/[draftId]/upload/init validates ownership/MIME/size/count from JSON metadata only and returns one presigned PUT URL per file (getPresignedPutUrl(), src/lib/r2/client.ts, 10-min TTL) at a server-generated draft-upload-raw/{draftId}/{uuid} key. The browser PUTs each file straight to Cloudflare R2 — no file bytes pass through any Vercel Function. POST /api/order-drafts/[draftId]/upload/complete HeadObject-verifies actual size/content-type, downloads each raw object, reuses the existing magic-byte check (matchesClaimedMimeType) and convertToPdf/mergePdfs unchanged, uploads the merged PDF to the existing draft-uploads/{draftId}/original.pdf key, calls the existing setDraftFile(), then deletes the raw objects. Idempotent: a retried complete call whose final object already exists in R2 (verified via HeadObject, not just the DB row) returns success without re-converting or touching raw objects again. Shared ownership/MIME/key-format logic now lives in src/lib/order-drafts/upload-shared.ts (server-only) and src/lib/order-drafts/upload-constants.ts (isomorphic, also imported by OrderForm.tsx), so frontend, init, complete, and the legacy endpoint cannot drift on limits. The old POST /api/order-drafts/[draftId]/upload (single multipart request) is kept only as a legacy fallback for cached old frontend bundles and is no longer called by the current frontend. The daily /api/cron/cleanup cron gained cleanupOrphanedRawUploads() (sweeps draft-upload-raw/ objects older than 24h via a new listObjectsByPrefix() R2 helper) and a bugfix: it used to return early (skipping cleanupExpiredOrderDrafts entirely) whenever there were zero expired documents rows.

**Rationale:**  
Files above ~4.5 MB triggered Vercel's hard Function payload limit (413 FUNCTION_PAYLOAD_TOO_LARGE) on the old single-request upload endpoint, even though the product's stated limit is 50 MB (authenticated) / 20 MB (anonymous) and the app-level size checks were never actually reached for large files. Moving the file bytes to a direct browser-to-R2 PUT removes Vercel's request body from the path entirely while keeping every existing business rule (MIME allow-list, per-file/total size caps, magic-byte validation, convertToPdf/mergePdfs, setDraftFile) completely unchanged.

**Impacted files/docs:**  
src/app/api/order-drafts/[draftId]/upload/init/route.ts, src/app/api/order-drafts/[draftId]/upload/complete/route.ts, src/app/api/order-drafts/[draftId]/upload/route.ts, src/lib/order-drafts/upload-shared.ts, src/lib/order-drafts/upload-constants.ts, src/lib/order-drafts/service.ts, src/lib/r2/client.ts, src/app/api/cron/cleanup/route.ts, src/components/order/OrderForm.tsx, messages/*/order.json (14 locales), docs/ai-context/30_ARCHITECTURE_OVERVIEW.md, docs/ai-context/70_DATABASE_AND_API_SURFACE.md

**Risks / caveats:**  
No DB-level atomic claim guards the narrow window where two concurrent complete calls both pass the idempotency check before either finishes — worst case is duplicate (non-corrupting) conversion work and a harmless duplicate write to the same final R2 key, not data corruption. R2 bucket CORS (PUT + Content-Type from the production and staging origins) must be configured directly in Cloudflare — outside this repo — before the direct PUT will work in a real browser; not yet done for production.

---

### 2026-07-14 — Direct-to-R2 upload flow for dashboard card-payment orders (upload-card init/complete)

**Decision:**  
Extended the direct-to-R2 pattern (already used for order-drafts) to the dashboard/card-payment upload flow, which was still hitting Vercel 413 FUNCTION_PAYLOAD_TOO_LARGE for files over ~4.5 MB. Added POST /api/documents/upload-card/init (auth + terms-accepted + Halyk-enabled + rate-limit + business-field validation, returns one presigned PUT URL per file at a server-generated card-upload-raw/{userId}/{uploadAttemptId}/{uuid} key) and POST /api/documents/upload-card/complete (HeadObject-verify, download, magic-byte check, convertToPdf/mergePdfs unchanged, upload the merged PDF to documents/{userId}/{uploadAttemptId}/original.pdf, then create document+job+quote via the new createCardOrder() in src/lib/documents/upload-card-shared.ts, which is the legacy route's business-logic tail moved verbatim). uploadAttemptId is client-echoed from init and doubles as documents.id, giving real idempotency without a migration: findExistingCardOrder() checks for an existing document+job before doing any R2 work and replays the existing job/price on a retry instead of creating a duplicate order. Moved MAX_FILE_SIZE_EACH/MAX_TOTAL_SIZE/UploadFormSchema/getAuthUser/getClientIp out of the legacy route.ts into the new shared module (route.ts now imports them) because Next.js's route-export type contract rejects any export from a route.ts file other than HTTP methods and a few reserved config exports — discovered by a failed typecheck when exporting them directly. Added a new generic src/lib/r2/upload-key-utils.ts (buildRawKey/isValidRawKey over an arbitrary {prefix}/{scope}/{uuid}) and refactored order-drafts/upload-shared.ts's key builder/validator to delegate to it, so both upload flows share one key-format implementation. The legacy POST /api/documents/upload-card (single multipart request) is unchanged in external behavior and kept only for cached old frontend bundles; the current dashboard frontend (OrderForm.tsx, mode='dashboard') now uses init/PUT/complete exclusively.

**Rationale:**  
The order-drafts (anonymous /start wizard) upload flow was already migrated to direct-to-R2 in an earlier session, but the dashboard's card-payment upload (documents/upload-card) is a separate code path with different business rules (full auth required, no anonymous session/consent-on-draft concept, terms-accepted + Halyk-enabled gates, document+job created immediately rather than deferred to a later convert step) and was still sending full multipart file bodies through a Vercel Function, so logged-in users hit the same 413 on files over ~4.5 MB despite the dashboard's stated 25 MB/file, 50 MB/total limits. Moving the file bytes to a direct browser-to-R2 PUT removes Vercel's request body from the path while reusing every existing business rule unchanged.

**Impacted files/docs:**  
src/app/api/documents/upload-card/init/route.ts, src/app/api/documents/upload-card/complete/route.ts, src/app/api/documents/upload-card/route.ts, src/lib/documents/upload-card-shared.ts, src/lib/r2/upload-key-utils.ts, src/lib/order-drafts/upload-shared.ts, src/components/order/OrderForm.tsx, messages/*/order.json (14 locales, dashboard.errors block), docs/ai-context/30_ARCHITECTURE_OVERVIEW.md, docs/ai-context/70_DATABASE_AND_API_SURFACE.md

**Risks / caveats:**  
Same non-corrupting concurrent-retry race window as the order-drafts flow: two near-simultaneous /complete calls with the same uploadAttemptId could both pass the idempotency check before either finishes, in the worst case creating duplicate (non-corrupting) work; no DB-level atomic claim was added since createCardOrder's document insert already gives an effective de-dupe boundary once one call completes. R2 bucket CORS (see prior decision) needs the same PUT+Content-Type rule — it already covers this flow since both use the same bucket/origins, no new CORS entry required.

---

### 2026-07-21 — New-model pricing formula (2026-Q3-KZ-NEWMODEL draft): billable pages, delivery, and urgency corrected (2026-07-21)

**Decision:**  
Real-world testing via tools/pricing-cli of the 2026-07-17 flat T/O/N/C/P/W/M formula (still DRAFT, never activated) surfaced three formula-level errors, now fixed in src/lib/pricing/calculator.ts and types.ts: (1) Billable translation pages: characterPages = charactersWithSpaces / 1800 (unrounded); billableTranslationPages = max(1, reliablePhysicalPageCount, characterPages) — whichever wins decides both the amount (physical pages × rate, or characters × rate / 1800) and the reported translationPageBasis ('minimum_one_page' | 'physical_pages' | 'character_count'). DOCX gets physicalPageCount=null (never a fabricated guess) when the page-render fails; tools/pricing-cli supports --manual-physical-pages / manualPhysicalPageCountOverride for operators to supply a real count. (2) Delivery/courier: deliveryRequired and fulfillmentMethod must always agree — calculatePrice() now throws PRICING_CONFIG_INVALID on contradiction; tools/pricing-cli's SAFE_DEFAULTS no longer pre-fills fulfillmentMethod='pickup' (that pre-fill was silently shadowing deliveryRequired-only manifests). Courier (5000 KZT) enters the WPO coordination base (W = (T+N+C) × rate) whenever fulfillmentMethod='delivery'. (3) Urgency: the same_day multiplier (x1.5 after_noon, x2 after_18) now multiplies the WHOLE standard retail (standardRetailKzt, computed with no urgency in the picture) to produce retailKzt — not just the WPO coordination fee as before. External payouts (translator/notary/courier) are never urgency-multiplied. Referral discount and partner commission are computed from retailKzt (post-urgency), never standardRetailKzt.

**Rationale:**  
The pre-2026-07-21 formula underbilled physical-page-heavy documents (ignored physical page count entirely), silently dropped the courier fee from real orders despite the report showing 'Доставка', and made same-day/rush notarization barely more expensive than standard — none of which matched the WPO-approved worked examples once tested against real files. This is a DRAFT-model correction only: the row is still status='draft', never activated, and production/checkout is unaffected (computeQuoteForJob, the function that would wire this into checkout, has zero callers today).

**Impacted files/docs:**  
src/lib/pricing/calculator.ts, src/lib/pricing/types.ts, tools/pricing-cli/lib/config.ts, tools/pricing-cli/lib/params-resolver.ts, tools/pricing-cli/lib/russian-report.ts, tools/pricing-cli/lib/summary.ts, tools/pricing-cli/lib/cli-args.ts, tools/pricing-cli/lib/pricing-run.ts, tools/pricing-cli/fixtures.ts, src/lib/document-analysis/analyze.ts, supabase/migrations/0058_pricing_versions_formula_version_2026_07_21.sql

**Risks / caveats:**  
Migration 0058 (metadata.formula_version bump on the draft row only) is PREPARED, not applied — the user applies Supabase migrations manually. Do not activate this pricing version and do not wire it into checkout until the user has manually re-tested via tools/pricing-cli.

---

### 2026-07-22 — New pricing model connected to Official/Notary checkout, gated by feature flags (2026-07-22)

**Decision:**  
computeQuoteForJob() (src/lib/pricing/service.ts) now gates non-electronic serviceLevels behind ENABLE_NEW_OFFICIAL_PRICING/ENABLE_NEW_NOTARY_PRICING (both default false, plain env vars mirroring getHalykConfig()'s pattern — no DB feature-flag table). When a flag is off, it refuses to quote (SERVICE_LEVEL_PRICING_DISABLED) rather than falling back to any prior formula, since production's main branch has a single monolithic legacy calculatePrice() with no calculateOfficialNotaryPrice split at all — there is no old official/notary path left in this codebase to fall back to. When a flag is on, the active pricing_versions row must be exactly '2026-Q3-KZ-NEWMODEL' with metadata.formula_version='new_2026_07_21', else PRICING_VERSION_MISMATCH — never silently prices against a stale row (e.g. the MVP row's marketing_rate_direct=0.10/owner_reserve_rate=0.07/mrp_value=3.69 instead of the approved 0.05/0.00/4.325). createCardOrder() (upload-card-shared.ts), the legacy upload-card/route.ts, and calculateDraftPrice()/convertDraftToOrder() (order-drafts/service.ts) now run real document analysis (src/lib/document-analysis/service.ts's resolveDocumentAnalysisForPricing(), idempotent via document_analysis's revision/status state machine — reuses a completed revision, refuses a second analysis while one is in flight, never re-runs OCR on retry) before computing a price, replacing the old hardcoded physicalPageCount=1 default that meant T (translation) was always 0 for non-electronic orders. order_drafts (pre-document, anonymous pricing) uses its own analysis_snapshot cache (migration 0061) since document_analysis requires a real documents.id that doesn't exist yet at draft stage. price_quotes gained formula_version (migration 0059, snapshotted at save time) and a BEFORE UPDATE trigger (migration 0060) that raises if status='paid' and the price snapshot (amount_kzt/wpo_financial_breakdown_json/etc.) changes — markQuotePaid() also gained the status guard markQuotePaymentPending() already had. A new shared FinancialReportModel (src/lib/pricing/financial-report.ts, synced worker copy at worker/src/lib/jira/financial-report.ts) now backs both the CLI's local .report.md and the Jira pricing breakdown description (worker/src/lib/jira/price-breakdown.ts, gated by ENABLE_NEW_JIRA_PRICING_REPORT, actually wired unlike the vestigial OFFICIAL_WORKFLOW_ENABLED) — replacing three independently-drifting re-implementations with one Russian-language model matching the CLI's already-approved report structure.

**Rationale:**  
Real-world testing surfaced that Official/Notary checkout was pricing every order with physicalPageCount hardcoded to 1 and no character count at all (T=0), and that production's live formula for these service levels is a completely different, older monolithic calculator than what this branch's calculator.ts now contains — deploying without a gate would silently switch live customer pricing to the new formula against whatever pricing_versions row happens to be active, with no way to control the rollout. Feature flags let the corrected formula and the corrected pricing_versions row activate together, as one controlled step, instead of by accident on deploy.

**Impacted files/docs:**  
src/lib/pricing/service.ts, src/lib/pricing/feature-flags.ts, src/lib/pricing/financial-report.ts, src/lib/document-analysis/service.ts, src/lib/documents/upload-card-shared.ts, src/app/api/documents/upload-card/route.ts, src/lib/order-drafts/service.ts, src/lib/order-drafts/types.ts, worker/src/lib/jira/financial-report.ts, worker/src/lib/jira/price-breakdown.ts, worker/src/lib/integrations.ts, tools/pricing-cli/lib/russian-report.ts, tools/pricing-cli/lib/summary.ts, supabase/migrations/0059-0061
...
### 2026-08-10 — Telegram Operations Jira fields hardcoded (no env vars); page-count/payout populated at issue-creation time

**Decision:**  
Simplified the Telegram Operations Jira field design per explicit correction: (1) all 9 field IDs (customfield_10123..10131) hardcoded directly into JIRA_FIELDS in src/lib/jira/client.ts and worker/src/lib/jira/order-fields.ts (kept in sync manually, same convention as every other Jira custom field in this codebase) -- no env vars for field IDs, src/lib/telegram-ops/jira-fields.ts is now just a role-dispatch layer over these constants. (2) Renamed the page-count field to 'Оплачиваемые страницы' and re-sourced it to price_quotes.translation_page_count_exact (the pricing engine's authoritative payable/billable count) instead of physical_page_count. (3) Split the shared payout field into two role-specific fields, 'Выплата переводчику' (customfield_10130) and 'Выплата нотариусу' (customfield_10131), both populated together at issue-creation time since both values are known upfront from the same quote computation -- avoids any handoff/overwrite timing dependency between the translator and notary phases. (4) All 9 fields are Text fields; the 3 pricing fields hold numeric values serialized as text (String(value)), parsed back via the new extractNumericTextValue() helper. (5) Kept Message ID fields (in-place message editing, per the original explicit UX requirement to never spam multiple messages) and User ID fields (required for start/done ownership enforcement) -- neither was removed, both were confirmed genuinely necessary rather than convenience. (6) Populating the 3 pricing fields required a targeted, explicitly-approved exception to the DOCX pipeline freeze's 'Jira/Google Drive integration workflow' line: a new standalone loadTelegramOpsPricingFields(jobId) in worker/src/lib/integrations.ts (read-only, wrapped in try/catch matching sibling helpers, never touches loadPriceBreakdownData() or any other pricing/rendering logic) reads price_quotes.translation_page_count_exact and cost_reservations.amount_kzt (translator_payout/translator_reserved_cost, notary_payout -- excluding status='canceled' rows per explicit instruction), threaded through createJiraIssue()'s existing params into buildJiraIssueFields(). The web-app copy of initializeOrderIntegrations() (src/lib/integrations/workflow.ts) is confirmed dead code (never called) so only the worker copy needed this change.

**Rationale:**  
The prior design used env vars for field IDs (defensible when the real Jira field IDs didn't exist yet) and a shared payout field (a design flaw once payout needed to be authoritative rather than illustrative) and physical_page_count (the wrong page-count concept -- billing-relevant page count, not raw PDF pages). Once the 9 Jira fields were actually created, hardcoding matches the codebase's own established convention for every other Jira custom field, and the split-payout-fields-populated-upfront design removes a timing dependency the shared-field version had. The pricing-pipeline touch was scoped as narrowly as possible (one new standalone read-only function, zero changes to OCR/translation/rendering/other Jira-issue fields) specifically because it required an explicit freeze exception, not a general pipeline change.
