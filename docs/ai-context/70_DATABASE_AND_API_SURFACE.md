...
| `partner_applications` | `partner_type`, `name`, `email`, `phone`, `organization`, `message`, `ref_code`, `utm_*`, `status` (`pending\|reviewing\|approved\|rejected`), `jira_issue_key`, `jira_issue_url`, `jira_sync_status` (`pending\|synced\|failed`), `jira_error`, `jira_created_at`, `approved_partner_id` (FK → partners.id), `approved_at`, `approved_by` — public partner program form submissions. RLS enabled, no browser policies. Migrations `0029`, `0030`, `0033`. |
| `partners` | `referral_code` (unique), `partner_type`, `email` (unique), `commission_rate`, `is_active`, `application_id`, `client_discount_enabled` (bool), `client_discount_type` (`percent\|fixed`), `client_discount_value`, `client_discount_min_order_amount`, `client_discount_max_amount` — approved partner accounts. RLS enabled, service-role only. Migrations `0029`, `0032`. |
| `partner_referrals` | `partner_id`, `job_id` (unique where not null), `user_id`, `ref_code`, `utm_source/medium/campaign/content/term`, `order_amount_kzt` (gross pre-discount price), `client_discount_applied_kzt` (discount subtracted at order creation), `commission_rate` (snapshot), `commission_base_kzt` (excl. discount + pass-throughs), `commission_kzt`, `status` (`pending\|confirmed\|refunded\|canceled\|paid\|excluded`), `payout_id` — tracks referral code attribution to orders. Migrations `0029`, `0031`, `0032`. Commission base = `order_amount_kzt − client_discount_applied_kzt − pass_throughs`. Pass-through exclusions: `notary_official_fee`, `delivery_fee` from `price_quote_items`. |
...
| GET | `/api/admin/partners/applications` | Operator — lists pending/reviewing applications (Bearer CRON_SECRET). |
| POST | `/api/admin/partners/approve-application` | Operator — creates partner row, updates application to `approved` (Bearer CRON_SECRET). Auto-generates referral code from org/name if not provided. Only way to create active partners — no public endpoint can do this. |
