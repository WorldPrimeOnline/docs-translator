...
| `partner_applications` | `partner_type`, `name`, `email`, `phone`, `organization`, `message`, `ref_code`, `utm_*`, `status` (`pending\|reviewing\|approved\|rejected`), `jira_issue_key`, `jira_sync_status` — public partner program form submissions. RLS enabled, no browser policies. Migration `0029`. |
...
| `partner_referrals` | `partner_id`, `job_id`, `user_id`, `ref_code`, `utm_*`, `commission_base_kzt`, `commission_kzt`, `status` (`pending\|completed\|excluded\|refunded`), `payout_id` — tracks referral code attribution to orders. Migration `0029`. |
