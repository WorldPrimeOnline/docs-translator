...
## Freedom Pay (staging card payments in KZT, test merchant #588913)

Implemented in `src/lib/payments/freedompay/` (client, config, signature, result-ack, status-map, xml) — independent of `BUSINESS_PROFILE.cardPaymentsActive` (that flag is Halyk-only; Freedom Pay's test merchant matches the current entity). Gated by `FREEDOMPAY_ENABLED` + `FREEDOMPAY_MERCHANT_ID` + `FREEDOMPAY_SECRET_KEY`. `checkout-provider.ts` routes staging (`NEXT_PUBLIC_APP_ENV=staging`) checkout through Freedom Pay; production still uses Halyk (currently disabled, see above). No fiscalization calls anywhere in this integration — explicit scope exclusion, separate future task.

API routes:
- `POST /api/payments/freedompay/initiate` — creates `payment_transactions` row, calls `init_payment`, returns redirect URL
- `POST /api/payments/freedompay/result` — public Result URL webhook (no session) — Freedom Pay's server-to-server callback
- `GET /api/payments/freedompay/status/[paymentId]` — session-authed on-demand reconciliation, polled by `/payment/result`
- `GET /api/cron/reconcile-freedompay-payments` — server-side fallback reconciliation (added 2026-08-21), separate from and does not modify `reconcile-payments` (Halyk-only). Triggered from the Railway worker every 15 min via `triggerReconcileFreedomPayPayments()` (`worker/src/lib/fiscal-reconciliation.ts`), same cadence as the Halyk trigger. Selects `payment_provider='freedom_pay'`, `status IN ('payment_pending','requires_review')`, calls `get_status3.php`, finalizes via `finalize_payment_transaction` on success.

**Vercel Deployment Protection — Result URL bypass (2026-08-21)**: staging's `*.vercel.app` deployment sits behind Vercel Authentication (SSO) for every route (`ssoProtection.deploymentType = all_except_custom_domains` — staging has no custom domain; production's `wpotranslations.org` is exempt). An unauthenticated Freedom Pay Result URL POST was previously redirected (302) to `vercel.com/sso-api` before ever reaching the Next.js route — confirmed via a live audit on a real successful payment (`0f15adf3-524c-4d83-bfe6-1d3f310d6b43`): no `POST /api/payments/freedompay/result` entry in Vercel logs, only `initiate` and the customer-triggered `status` poll. Fix: `getFreedomPayResultUrl()` (`src/lib/payments/freedompay/config.ts`) appends `?x-vercel-protection-bypass=<VERCEL_AUTOMATION_BYPASS_SECRET>` to `pg_result_url` — staging only (`NEXT_PUBLIC_APP_ENV`/`APP_ENV` ≠ `'production'`) and only when the secret env var is present; production's Result URL is never modified. `VERCEL_AUTOMATION_BYPASS_SECRET` is Vercel's own auto-injected env var for an existing "Protection Bypass for Automation" secret on the project — nothing new provisioned. The query param never affects either signature: `script_name` for both outbound requests and inbound Result URL verification is derived from the constant `FREEDOMPAY_RESULT_PATH`, never from the live request URL.

**`callback_received_at` / `status_checked_at` — ownership fixed (2026-08-21, migration 0071)**: `finalize_payment_transaction` (migration 0070) previously set `callback_received_at = NOW()` unconditionally, and was called from both the real Result URL route AND the on-demand status route's `get_status3.php` reconciliation — so the column could not distinguish "Freedom Pay actually delivered the callback" from "we polled it ourselves." Migration 0071 (`CREATE OR REPLACE FUNCTION`, no table/schema change) removes that write from the RPC entirely. `callback_received_at` is now stamped **only** by `result/route.ts`, immediately after a signature-verified POST matches a known payment row — before any branching, so idempotent retries also count. `status_checked_at` is now persisted **unconditionally** after every successful `get_status3.php` call in `result/route.ts`, `status/route.ts`, and the new reconciliation cron — including the `paid` path, which previously skipped the write and fell straight through to the finalize RPC.
